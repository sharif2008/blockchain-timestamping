$(document).ready(function () {
    $("#verifyButton").click(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    console.log((response.data[2] == hash) ? 'match' : 'Not match'); $('.alert').removeClass('d-none');
    ...
    $("#signButton").click(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    alert(response.msg);
                    $('#txUuid').html(response.data.uuid); $('#txHash').html(response.data.tx);
                    let link = "https://ropsten.etherscan.io/tx/" + response.data.tx; link = '<a  href = "' + link + '" target = "_blank" > ' + link + ' </a > '; link += '(Click the link to check mining status)'; $('#txLink').html(link);
                } else {
                    alert(response.msg);
                }
            },
    ...
    $(".document").change(function () {
        ...
        if (files.length > 0) {
            fd.append('document', files[0]); $.ajax({
                url: '/file/upload', type: 'post',
                data: fd, contentType: false, processData: false, success: function (response) {
                    if (response.status) {
                        console.log(documentSelector.attr("class")); if
                            (documentSelector.hasClass('upload')) {
